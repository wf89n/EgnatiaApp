...
class Group(models.Model):
    ...
    def calculate_total_expenses(self):
        """Calculate total expenses based on all department costs."""
        # Sum the expenses of all departments, including 'Ergates'
        return sum(
            group_department.total_department_cost()
            for group_department in GroupDepartment.objects.filter(group=self)
        )
    ...
    def get_departments_costs(self):
        ...
        department_costs = {}
        group_departments = GroupDepartment.objects.filter(group=self)
        for group_department in group_departments:
            department_costs[group_department.department.name] = group_department.total_department_cost()
...
class GroupDepartment(models.Model):
    ...
    def total_department_cost(self):
        ...
        employees = Employee.objects.filter(
            role__department=self.department,
            role__department__groups=self.group
        )

        total = 0
        for employee in employees:
            employee_pay = employee.basic_info.total_pay()
            print(f"Employee: {employee.basic_info.first_name} {employee.basic_info.last_name} - Pay: {employee_pay}")
            total += employee_pay
        print(f"Total cost for department {self.department.name}: {total}")
        return total
...
class BasicInfo(models.Model):
    ...
    def save(self, *args, **kwargs):
        # Prevent saving of non-existent or manually assigned group
        if self.group is not None:
            super().save(*args, **kwargs)
        else:
            # Handle the case where group is None (e.g., raise an exception or just pass)
            pass
